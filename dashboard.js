...
chrome.storage.local.get(['theme', 'aiModel'], (data) => {
    ...
    const VALID_MODELS = ['gemini-2.5-flash', 'gemini-2.5-pro', 'gemini-2.5-flash-lite', 'gemini-2.0-flash'];
...
function renderSidebar(conversations) {
    ...
    if (!conversations || conversations.length === 0) {
        convList.innerHTML = '<div class="conv-empty">עדיין אין שיחות.<br>התחל שיחה חדשה 👆</div>';
        return;
    }
...
function rebuildChatFromHistory(messages) {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (m.role === 'user') {
            ...
            if (!trimmed.startsWith('✅') && !trimmed.startsWith('⏳') && !trimmed.startsWith('🚨') && !trimmed.startsWith('שגיאת') && !trimmed.startsWith('⚠️')) {
            ...
            try {
                ...
                // Not JSON, raw text
                if (rawText.trim().length > 0) pairs.push({ role: 'model', text: rawText });
            }
        }
    }
...
function liveUpdatePills(workersMap) {
    ...
    chatWindow.querySelectorAll('.agent-pill[data-worker-id]').forEach(pill => {
        ...
        if (prefix) {
            ...
            else if (w.status === 'error' || w.status === 'cancelled') prefix.textContent = 'שגיאה ב:';
        ...
        if (spinner && (w.status === 'done' || w.status === 'error' || w.status === 'cancelled')) {
            spinner.remove();
            if (!pill.querySelector('.pill-done-icon, .pill-err-icon')) {
                const icon = document.createElement('span');
                icon.className = w.status === 'done' ? 'pill-done-icon' : 'pill-err-icon';
                icon.textContent = w.status === 'done' ? '✓' : '✗';
                pill.insertBefore(icon, pill.querySelector('.pill-status-prefix') || pill.firstChild);
            }
        }
        ...
        const resumeContainer = panelEl ? panelEl.querySelector('.pill-resume-container') : null;
        ...
        if (w.status === 'paused') {
            ...
            if (resumeContainer) {
                ...
                if (msgEl) msgEl.textContent = w.errorMsg || 'סיים את הפעולה הדרושה בטאב הפתוח ולחץ "סיימתי".';
            }
...
function addMessageUI(text, isUser = false, isProgress = false, spawnedWorkers = null, instant = false) {
    ...
    if (isProgress) {
        ...
        function simpleMarkdownParser(md) {
            ...
            html = html.split('\n').map(line => {
                ...
                return line;
            }).join('<br>').replace(/(<br>\s*){2,}/g, '<br><br>');
            return html;
        }
...
function buildAgentPill(workerId) {
    ...
    resumeBtn.addEventListener('click', () => {
        ...
        chrome.runtime.sendMessage({ action: 'resume_worker', workerId: workerId, input: resumeInput.value });
    });
...
function updatePillThinking(workerId, logs, status) {
    ...
    if (!logs || logs.length === 0) {
        stepsEl.innerHTML = '<div style="padding:8px 0;color:var(--text-dim);font-size:0.82rem;">אין פעולות עדיין...</div>';
    ...
    const actionIcons = {
        open_url: '🌐', read_page: '📄', click_element: '🖱️', type_in_editor: '✏️',
        search: '🔍', scroll: '↕️', done: '✅', wait: '⏳',
        key_press: '⌨️', editor_format: '🎨', analyze_editor: '🔬',
    };

    stepsEl.innerHTML = logs.map((log, i) => {
        const isFail = String(log.message || '').includes('❌') || String(log.message || '').toLowerCase().includes('error');
        const isLast = i === logs.length - 1;
        const dotClass = isFail ? 'fail' : (isLast && status === 'running') ? 'running' : 'done';
        const icon = actionIcons[log.action] || '⚡';
        const title = toTitleCase(log.action || 'פעולה');
        const body = log.thought ? log.thought.substring(0, 120) + (log.thought.length > 120 ? '...' : '') : '';
        const msgSnippet = log.message ? String(log.message).substring(0, 60) : '';
...
function doSend(text) {
    ...
    chrome.runtime.sendMessage(options, (response) => {
        ...
        if (chrome.runtime.lastError || !response) {
            addMessageUI('❌ שגיאת תקשורת: ' + (chrome.runtime.lastError?.message || 'Unknown'), false);
            return;
        }
        if (response.error) {
        ...
        addMessageUI(finalText, false, false, spawnedIds.length ? spawnedIds : null);
...
function finalizePills(spawnedIds) {
    chrome.storage.local.get(['activeWorkers'], (data) => {
        ...
        spawnedIds.forEach(wid => {
            const w = aw[wid];
            const pill = chatWindow.querySelector(`.agent-pill[data-worker-id="${wid}"]`);
            if (!pill) return;
            ...
            if (prefix) prefix.textContent = status === 'done' ? 'סיים:' : status === 'error' ? 'שגיאה ב:' : 'ממתין ל:';
...
function getStatusLabel(status) {
    const map = { running: 'פעיל', done: 'סיים ✓', paused: 'ממתין לאדם', error: 'שגיאה', cancelled: 'בוטל' };
    return map[status] || status;
}

function getBadgeClass(status) {
    const map = { running: 'badge-running', done: 'badge-done', paused: 'badge-paused', error: 'badge-error', cancelled: 'badge-cancelled' };
...
function generateThinkingHTML(logsArray) {
    ...
    return logsArray.map(log => {
        ...
        const title = toTitleCase(log.action || 'פעולה');
        const body = log.thought ? log.thought.substring(0, 180) + (log.thought.length > 180 ? '...' : '') : '';
...
function buildAgentCard(id, worker) {
    ...
    header.innerHTML = `
        ...
        <div class="card-header-right">
            ...
            ${isRunning ? `<button class="stop-btn" data-worker-id="${id}" title="עצור סוכן">
    ...
    const pbFill = document.createElement('div'); pbFill.className = 'progress-bar-fill';
    pbWrap.appendChild(pbFill); card.appendChild(pbWrap);
    ...
    // Error
    if ((worker.status === 'error' || worker.status === 'cancelled') && worker.errorMsg) {
...
function updateExistingCard(id, worker) {
    ...
    if (badge) { badge.className = `status-badge ${getBadgeClass(worker.status)}`; badge.textContent = getStatusLabel(worker.status); }
    ...
    if (urlEl && worker.url && urlEl.textContent !== worker.url) {
        urlEl.textContent = worker.url; urlEl.title = worker.url;
    }
    ...
    if (worker.status === 'paused' && !existingResume) card.appendChild(buildResumeBox(id, worker.errorMsg));
    else if (worker.status !== 'paused' && existingResume) existingResume.remove();

    let existingErr = card.querySelector('.card-error-msg');
    if ((worker.status === 'error' || worker.status === 'cancelled') && worker.errorMsg) {
        if (!existingErr) { const e = document.createElement('div'); e.className = 'card-error-msg'; e.innerText = worker.errorMsg; card.appendChild(e); }
        else if (existingErr.innerText !== worker.errorMsg) existingErr.innerText = worker.errorMsg;
    }
...
function renderWorkers(workersMap) {
    ...
    const ids = defaultIds.filter(wid => !workersMap[wid].conversationId || workersMap[wid].conversationId === activeConvId);

    if (ids.length === 0) {
        Array.from(agentsGrid.querySelectorAll('.agent-card')).forEach(card => card.remove());
    ...
    const runningCount = ids.filter(id => workersMap[id].status === 'running').length;
    const totalCount = ids.length;
    activeCountEl.textContent = runningCount > 0 ? `${runningCount} פעילים / ${totalCount} סה"כ` : `${totalCount} סוכנים`;
    ...
    ids.forEach(id => {
        if (!updateExistingCard(id, workersMap[id])) agentsGrid.appendChild(buildAgentCard(id, workersMap[id]));
    });
}
...
agentsGrid.addEventListener('click', (e) => {
    ...
    if (btnResume) {
        ...
        const input = btnResume.closest('.card-resume-box').querySelector('.resume-input');
        const humanMessage = input ? input.value.trim() : '';
        btnResume.disabled = true; btnResume.textContent = 'מתחיל...';
        chrome.runtime.sendMessage({ action: 'resume_worker', workerId, humanMessage }, () => { if (input) input.value = ''; });
        return;
    }
...
document.getElementById('hard-reset-btn').addEventListener('click', () => {
    if (confirm('האם למחוק לחלוטין את כל הזיכרון? זה יאפס את כל השיחות, ההיסטוריה והסוכנים הפעילים.')) {
...
document.getElementById('export-logs-btn').addEventListener('click', () => {
    chrome.storage.local.get(['activeWorkers'], (data) => {
        ...
        const blob = new Blob([JSON.stringify({ timestamp: new Date().toISOString(), workers }, null, 2)], { type: 'text/plain;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = `swarm_logs_${Date.now()}.txt`;
...
function openCanvas(text) {
    ...
    if (h1Match) {
        ...
        if (firstLine.length > 5) title = firstLine.substring(0, 40) + (firstLine.length > 40 ? '...' : '');
    }
