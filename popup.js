const Sanitizer = {
    cleanHTML(htmlString) {
        ...
        const allowedTags = new Set(['P', 'BR', 'B', 'I', 'STRONG', 'EM', 'UL', 'OL', 'LI', 'A', 'CODE', 'PRE', 'H1', 'H2', 'H3', 'SPAN', 'DIV', 'DETAILS', 'SUMMARY']);
        ...
        while ((currentNode = walker.nextNode())) {
            ...
            for (let i = attrs.length - 1; i >= 0; i--) {
                ...
                const cleanAttrVal = attr.value.replace(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205f\u3000]/g, '').toLowerCase();
                if (!allowedAttrs.has(attrName) || cleanAttrVal.startsWith('javascript:')) {
...
async function checkSessionAndStart() {
    ...
    if (loggedInEmail) {
        chrome.runtime.sendMessage({ action: "login", interactive: false, expectedEmail: loggedInEmail }, (response) => {
...
if (loginBtn) {
    loginBtn.addEventListener('click', async () => {
        ...
        chrome.runtime.sendMessage({ action: "login", interactive: true, expectedEmail: email }, async (response) => {
...
if (exportLogBtn) {
    exportLogBtn.addEventListener('click', async () => {
        try {
            ...
            const logText = chatHistory.map(msg =>
...
async function loadChat() {
    ...
    if (chatHistory.length === 0) {
        fragment.appendChild(createMessageElement("שלום! המערכת מחוברת. מה נרצה לעשות היום?", false));
    } else {
...
function sendPrompt(text) {
    ...
    chrome.runtime.sendMessage({
        ...
        if (response?.csvCmd) {
            const exportMatch = response.csvCmd.match(/\[EXPORT_CSV\]\s*(.+?)\s*\|\s*([\s\S]+?)(?=\n\[|$)/i);
            if (exportMatch) {
                let filename = exportMatch[1].trim();
                if (!filename.endsWith(".csv")) filename += ".csv";
                const blob = new Blob(["\uFEFF" + exportMatch[2].trim()], { type: 'text/csv;charset=utf-8;' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
...
document.getElementById("send-btn").addEventListener("click", () => {
    if (input.value.trim()) { sendPrompt(input.value.trim()); input.value = ""; }
});
input.addEventListener("keypress", e => {
    if (e.key === "Enter" && input.value.trim()) { sendPrompt(input.value.trim()); input.value = ""; }
});
...
document.querySelectorAll('.quick-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
        ...
        if (url.startsWith('chrome://') || url.startsWith('edge://') || url.startsWith('about:')) {
            addMessageUI("⚠️ שגיאה: לא ניתן לבצע פעולות על דפי מערכת מוגבלים.", false, true);
            return;
        }
