...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "init_skill") {
    ...
    console.log(
      `[Content-Agent] Initialized with skill: ${request.skill}, id: ${request.agentId}`,
    );
  ...
  if (request.action === "read_page") {
    ...
    DOMUtils.waitForDOMQuiet(3000, 400).then(() => {
      ...
      let pageText = mainNode
        ? mainNode.innerText.replace(/\s+/g, " ").trim().substring(0, 2500)
        : "ריק.";
  ...
  if (request.action === "interact_element") {
    let el = window.__agentElementsMap
      ? window.__agentElementsMap.get(request.id)
      : null;
    if (!el) {
      sendResponse({
        success: false,
        feedback: `❌ שגיאה: ID ${request.id} לא נמצא. בצע read_page מחדש.`,
      });
      return true;
    }
    ...
    try {
      ...
      if (
        ...
        if (match) {
          ...
          sendResponse({
            success: true,
            feedback: `✅ בחרתי ב-SELECT: "${match.text}"`,
          });
        } else {
          sendResponse({
            success: false,
      ...
      if (
        ...
        sendResponse({
          success: true,
          feedback: `✅ ${el.type} [ID:${request.id}] ${el.checked ? "סומן" : "בוטל"}.`,
        });
        return true;
      }
      ...
      if (
        ...
        if (el.isContentEditable || el.getAttribute("role") === "textbox") {
          ...
          if (!el.innerText.includes(request.typeText))
            el.innerText = request.typeText;
        } else {
        ...
        sendResponse({
          success: true,
          feedback: `✅ הוקלד "${request.typeText.substring(0, 40)}" ב-[ID:${request.id}]${request.pressEnter ? " + Enter" : ""}`,
        });
        return true;
      }
      ...
      sendResponse({
        success: true,
        feedback: `✅ נלחץ בהצלחה אלמנט [ID:${request.id}].`,
      });
    } catch (e) {
      sendResponse({
        success: false,
        feedback: `❌ שגיאה באינטראקציה: ${e.message}`,
      });
    }
    return true;
  }
  ...
  if (request.action === "extract_data") {
    try {
      ...
      if (elements.length > 0) {
        ...
        sendResponse({
          success: false,
          text: "לא נמצאו נתונים התואמים לבקשה.",
        });
      }
    } catch (e) {
      sendResponse({ success: false });
  ...
  if (request.action === "key_press") {
    let el =
      request.id && window.__agentElementsMap
        ? window.__agentElementsMap.get(request.id)
        : document.activeElement;
    if (!el || el === document.body) {
      sendResponse({
        success: false,
        feedback: `❌ לא נמצא אלמנט ממוקד לשליחת מקשים. ניתן לציין id מה-read_page.`,
      });
      return true;
    }
    ...
    try {
      ...
      const eventInfo = {
        ...
        code: keyName,
        keyCode:
          keyName.length === 1 ? keyName.toUpperCase().charCodeAt(0) : 13,
        ctrlKey,
        shiftKey,
        altKey,
        metaKey: ctrlKey,
      ...
      sendResponse({
        success: true,
        feedback: `✅ הוקש [${request.key}] בהצלחה.`,
      });
    } catch (e) {
      sendResponse({
        success: false,
        feedback: `❌ שגיאה בשליחת מקש: ${e.message}`,
      });
    }
    return true;
  }
  ...
  if (request.action === "editor_format") {
    try {
      ...
      if (success) {
        sendResponse({
          success: true,
          feedback: `✅ הופעל עיצוב: ${command} ${value ? "עם הערך " + value : ""}`,
        });
      } else {
        ...
        if (
          ...
          if (success) {
            sendResponse({
              success: true,
              feedback: `✅ (אילוץ פוקוס) הופעל עיצוב: ${command}`,
            });
          } else {
            sendResponse({
              success: false,
              feedback: `❌ הפעלת עיצוב (${command}) נכשלה. ייתכן שאין טקסט מסומן בחלון או שהפקודה אינה נתמכת בעורך זה.`,
            });
          }
        } else {
          sendResponse({
            success: false,
            feedback: `❌ טכניקת עיצוב נכשלה. ודא שסמן העריכה נמצא בתוך תיבת הטקסט.`,
          });
        }
      }
    } catch (e) {
      sendResponse({
        success: false,
        feedback: `❌ שגיאה בעיצוב: ${e.message}`,
      });
    }
    return true;
  }
  ...
  if (request.action === "analyze_editor") {
    try {
      ...
      if (
        ...
        document
          .querySelectorAll(".components-button, .mce-btn")
          .forEach((btn) => {
            const label =
              btn.getAttribute("aria-label") ||
              btn.innerText ||
              btn.getAttribute("data-toolbar-item") ||
              btn.title;
            if (label && label.trim().length > 0)
              elements.push(`[WP Button] ${label.trim().replace(/\n/g, " ")}`);
          });
      }
      // 2. Notion
      else if (
        ...
        elements.push(
          "ניתן להשתמש בלוכסן '/' לפתיחת תפריט הבלוקים ופקודות הטקסט",
        );
      }
      // 3. Google Docs
        ...
        elements.push(
          "עורך קנבס ייעודי - ניתן לגשת לתפריטים העליונים דרך מבנה .docs-material-menu-button-inner-box",
        );
      }
        ...
        document
          ...
          .forEach((btn) => {
            const label =
              ...
              btn.innerText;
            if (label && label.trim().length > 0)
              elements.push(`[Editor Btn] ${label.trim().replace(/\n/g, " ")}`);
          });
      }
...
${uniqueElements.length > 0 ? "\nכפתורי עורך/סרגל כלים זמינים כרגע:\n" + uniqueElements.slice(0, 20).join("\n") : ""}`;
      ...
      sendResponse({
        success: false,
        text: `❌ שגיאה בניתוח סביבת העורך: ${e.message}`,
      });
    }
    return true;
  }
  ...
  if (request.action === "complex_click") {
    ...
    const searchInElements = (elements) => {
      ...
      for (let el of elements) {
        ...
        const innerTxt = (el.innerText || el.textContent || "")
          .toLowerCase()
          .trim();
        ...
        if (
          innerTxt.includes(txt) ||
          aria.includes(txt) ||
          title.includes(txt)
        ) {
          let matchText = innerTxt.includes(txt)
            ? innerTxt
            : aria.includes(txt)
              ? aria
              : title;
    ...
    let attempts = 0;
    const attemptClick = () => {
      let errClose = document.querySelector(
        'button[name="ok"], button[name="close"]',
      );
      ...
      if (txt === "שליחה" || txt === "send" || txt.includes("שלח")) {
        target = document.querySelector(
          'div[aria-label^="שליחה"], div[aria-label^="Send"], div[data-tooltip^="Send"][role="button"]',
        );
      }

      if (!target) {
        target = searchInElements(
          document.querySelectorAll(
            'button, a, [role="button"], [role="link"], div[role="button"], input[type="button"], div[aria-label="שליחה"], input[type="radio"]',
          ),
        );
      }
      if (!target) {
        target = searchInElements(
          document.querySelectorAll("span, div, li, label"),
        );
      }
      ...
      if (target && target.offsetParent !== null) {
        ...
        if (
          ...
          let radio =
            ...
            target.parentElement.querySelector('input[type="radio"]');
          if (radio) {
            radio.click();
            sendResponse({ success: true });
            return;
    ...
    const findInput = () => {
      if (lbl.includes("נמענים") || lbl.includes("to") || lbl === "אל") {
        ...
        return querySelectorDeep(
          '[role="combobox"], input[aria-label="To"], input[aria-label="נמענים"]',
        );
      }
      if (lbl.includes("נושא") || lbl.includes("subject")) {
        fieldType = "subject";
        return querySelectorDeep(
          'input[name="subjectbox"], input[placeholder="נושא"], input[placeholder="Subject"]',
        );
      }
      if (
        lbl.includes("תוכן") ||
      ...
      for (let l of labels) {
        const labelText = (l.innerText || l.textContent || "")
          .toLowerCase()
          .trim();
        if (labelText === lbl || labelText.includes(lbl)) {
          ...
          let associatedInput = l.querySelector(
            'input, textarea, [contenteditable="true"], [role="textbox"]',
          );
          if (associatedInput) return associatedInput;
          associatedInput = l.nextElementSibling?.querySelector(
            'input, textarea, [contenteditable="true"], [role="textbox"]',
          );
          if (associatedInput) return associatedInput;
        }
      ...
      for (let el of inputs) {
        ...
        if (attrs.some((a) => a === lbl || a.includes(lbl))) {
          target = el;
          break;
        }
      }
...
if (window.location.hostname.includes("youtube.com")) {
  ...
  const handleAds = () => {
    ...
    const specificSkipButtons = document.querySelectorAll(
      ".ytp-ad-text.ytp-ad-skip-button-text, .ytp-ad-skip-button-container *",
    );
    for (let el of specificSkipButtons) {
      if (el.offsetParent === null) continue;
      const text = (el.innerText || el.textContent || "").trim();
      if (
        ["דילוג", "דילוג על מודעות", "skip ads", "skip ad", "skip"].includes(
          text.toLowerCase(),
        )
      ) {
        el.click();
    ...
    const overlayAdCloseButton = document.querySelector(
