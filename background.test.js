...
describe("Background Script - Pure Functions", () => {
  describe("isRefusal", () => {
    it("should identify English AI refusals", () => {
      expect(
        sandbox.isRefusal("I'm sorry, but I can't fulfill this request."),
      ).toBe(true);
      expect(sandbox.isRefusal("I cannot provide this information.")).toBe(
        true,
      );
    });

    it("should identify Hebrew AI refusals", () => {
      expect(sandbox.isRefusal("אני מצטער, אבל אינני יכול לעשות זאת.")).toBe(
        true,
      );
    ...
    it("should ignore normal conversational text", () => {
      expect(sandbox.isRefusal("I successfully completed the task.")).toBe(
        false,
      );
  ...
  describe("parseJSON", () => {
    it("should parse valid raw JSON", () => {
      const data = sandbox.parseJSON(
        '{"action": "test", "parameters": {"key": "val"}}',
      );
    ...
    it("should extract JSON payload mixed with text using fallback regex", () => {
      ...
      expect(sandbox.parseJSON(raw)).toEqual({
        action: "extract",
        parameters: {},
      });
    });
    ...
    it("should return null for malformed JSON without brackets", () => {
      expect(
        sandbox.parseJSON("just standard text without json formatting"),
      ).toBeNull();
    });
  });
