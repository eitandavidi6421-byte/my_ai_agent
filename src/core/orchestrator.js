...
export class SwarmOrchestrator {
  ...
  enqueueTask(taskDef) {
    ...
    console.log(
      `[Orchestrator] Enqueued task ${taskId} (skill: ${fullTask.skill}, queue: ${this.taskQueue.length})`,
    );
  ...
  async processQueue() {
    ...
    try {
      while (
        ...
        this.activeAgents.size < MAX_CONCURRENT
      ) {
  ...
  async spawnAgent(taskDef) {
    ...
    console.log(
      `[Orchestrator] Spawning ${agentId} (${skill.name}) → ${taskDef.url}`,
    );
  ...
  async onWatchdogTimeout(agentId) {
    ...
    console.warn(
      `[Orchestrator] ⏰ Watchdog timeout for ${agentId} (${agent.skill})`,
    );
    ...
    if (newRetryCount < (agent.maxRetries || MAX_RETRIES)) {
      console.log(
        `[Orchestrator] Re-queueing task (retry ${newRetryCount}/${agent.maxRetries || MAX_RETRIES})`,
      );
      this.enqueueTask({
        skill: agent.skill,
      ...
      console.error(
        `[Orchestrator] Max retries exceeded for ${agentId}. Task failed.`,
      );
      emitAgentError(
        agentId,
        `Max retries (${agent.maxRetries || MAX_RETRIES}) exceeded`,
      );
  ...
  async startProject(userRequest, conversationId) {
    console.log(
      `[Orchestrator] Starting project for conversation ${conversationId}`,
    );
    const roadmap = await this.roadmap.generateRoadmap(
      userRequest,
      conversationId,
    );
    return roadmap;
  ...
  async continueProject(conversationId) {
    ...
    const previousResults =
      await this.roadmap.collectPreviousResults(conversationId);
  ...
  async _runAgentLoop(agentId, tabId, taskDef) {
    ...
    const alive = async () => {
      try {
        ...
        return (
          !!activeWorkers[agentId] &&
          activeWorkers[agentId].status !== "cancelled"
        );
      } catch {
        return true;
      }
    ...
    try {
      ...
      while (!done && loops < MAX_WORKER_STEPS) {
        ...
        try {
          ...
          obj = parseJSON(raw);
          if (!obj || !obj.action)
            throw new Error("Invalid JSON or missing action");
          messages.push({
            role: "model",
          ...
          messages.push({
            role: "user",
            parts: [
              { text: `Error: ${e.message}. Return valid JSON with action.` },
            ],
          });
          continue;
        }
        ...
        if (
          skill &&
          !isActionAllowed(taskDef.skill, obj.action) &&
          obj.action !== "done"
        ) {
          messages.push({
            role: "user",
            parts: [
              {
                text: `⚠️ Action "${obj.action}" is NOT allowed for your skill (${taskDef.skill}). Allowed actions: ${skill.allowedActions.join(", ")}. Use an allowed action.`,
              },
            ],
          });
          continue;
        ...
        if (timesRepeated === 3) {
          messages.push({
            ...
            parts: [
              {
                text: "🚨 You repeated the same action 3 times. Call done with whatever data you have — even partial.",
              },
            ],
          });
          ...
          await this.handleAgentError(
            agentId,
            `Infinite loop detected. Last actions:\n${partialInfo}`,
          );
          closeTab();
          return;
        }
      ...
      if (!done) {
        ...
        if (isRunning) {
          ...
          await this.handleAgentError(
            agentId,
            `Max steps exceeded (${loops}). ${partialReport || ""}`,
          );
        }
        closeTab();
      }
  ...
  async killAgent(agentId) {
    ...
    await updDirect(agentId, {
      status: "cancelled",
      errorMsg: "Killed by orchestrator",
    });
    this.activeAgents.delete(agentId);
