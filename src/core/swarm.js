...
import { isRefusal, getRefusalOverrideMessage, parseJSON, sanitizeHistory } from '../utils/helpers.js';
import { callGeminiAPI } from './api.js';
import { getWorkerPrompt, getManagerPrompt } from '../prompts/system-prompts.js';
import {
    waitForTab, scriptReadPage, scriptInteract, scriptClickText,
    scriptTypeText, scriptExtract, scriptKeyPress, scriptAnalyzeEditor,
    scriptEditorFormat, scriptRichTextType
...
export async function executeAction(actionObj, tabId) {
    ...
    if (action === 'open_url' && tabId && p.url) {
        ...
        if (!contentToWrite) {
            feedback = '❌ type_in_editor: חסר שדה formatted_markdown_content (או text).';
        } else {
            await new Promise(r => setTimeout(r, 1200));
            const res = await scriptRichTextType(tabId, contentToWrite, !!p.clear_first);
            if (res.ok) {
                await new Promise(r => setTimeout(r, 800));
...
// ═══════════════════════════════════════════════════════════
export async function runManagerOrchestrator(userMessage, conversationId, sendResponse) {
    const { loggedInEmail } = await chrome.storage.local.get(['loggedInEmail']);
    if (!loggedInEmail) return sendResponse({ error: 'אינך מחובר.' });
    ...
    try {
        ...
        // כך שאם המשתמש עובר לצ'אט אחר ואז חוזר, ההודעה לא תיעלם
        {
            ...
            earlyCh[conversationId] = [...earlyHist, { role: 'user', parts: [{ text: userMessage }] }];
        ...
        while (!done && loops < 30) {
            ...
            if (!obj) {
                ...
                messages.push({ role: 'user', parts: [{ text: `JSON שגוי. חובה להחזיר JSON בלבד בפורמט: { "thought": "...", "action": "...", "parameters": {} } — ללא קוד markdown, ללא הסברים. רק JSON.` }] });
                continue;
            }
...
// ═══════════════════════════════════════════════════════════
export async function runSwarmWorkerLoop(workerId, tabId, task, savedMessages = null) {
    ...
    try {
        ...
        while (!done && loops < 35) {
            ...
            const sameUrlCount = lastFewActions.filter(s => s === actionSignature).length;
            ...
            if (timesRepeated === 2) {
                ...
                const searchQuery = currentUrl
                    ? currentUrl.replace(/https?:\/\/[^/]+/, '').replace(/[-_/.]/g, ' ').trim()
                    : 'מידע רלוונטי';
                ...
                messages.push({ role: 'user', parts: [{ text: `🚨 [מערכת] אתה תקוע. שלח done עם המידע שיש לך עד כה — אפילו אם הוא חלקי. עדיף דיווח חלקי מאשר כלום.` }] });
            ...
            if (isRunning) {
                ...
                const partialReport = currentLogs.length > 0
                    ? `⏰ הגעתי למגבלת סיבובים (${loops} סיבובים). מידע חלקי שנאסף:\n` +
                      currentLogs.filter(l => l.action === 'done' || l.message?.length > 20)
 ...
 */
export async function runAgentlessResearch(userMessage, conversationId, mode, sendResponse) {
    const { loggedInEmail } = await chrome.storage.local.get(['loggedInEmail']);
    if (!loggedInEmail) return sendResponse({ error: 'אינך מחובר.' });
    ...
    try {
        ...
        if (mode === 'quick') {
            chrome.runtime.sendMessage({ action: 'manager_progress', text: 'מבצע מחקר מהיר ברשת...' });
...
הקפד לא לאבד שום קישור או מידע קריטי.`;
            ...
            finalResponse = await callGeminiAPI(prompt, messages, { useSearch: true });

        } else if (mode === 'deep') {
            chrome.runtime.sendMessage({ action: 'manager_progress', text: 'מתכנן מתווה למחקר עמוק (Deep Research)...' });
...
דוגמה: ["query 1", "query 2", "query 3"]`;
            ...
            let queriesJsonRaw = await callGeminiAPI(queryGenPrompt, messages, { useSearch: false });
            let queries = [];
            try {
                queries = JSON.parse(queriesJsonRaw.replace(/```json/g, '').replace(/```/g, '').trim());
            } catch (e) {
                // התאוששות במקרה של כשל parsing
                queries = [userMessage, userMessage + " הסבר מורחב", userMessage + " מקרי בוחן"];
            }

            // חיתוך אם יצר המון:
            queries = queries.slice(0, 4);

            chrome.runtime.sendMessage({ action: 'manager_progress', text: `חוקר ${queries.length} כיוונים במקביל באמצעות Google Search...` });
...
שאילתה לחיפוש: ${query}`;
            ...
            chrome.runtime.sendMessage({ action: 'manager_progress', text: 'מבצע סינתזה וכותב דו"ח מחקר מורחב...' });
...
${searchResults.join('\n\n========================\n\n')}`;
            ...
            finalResponse = await callGeminiAPI(synthesisPrompt, messages, { useSearch: false });
        }
        ...
        try {
            ...
            hist[conversationId].push({ role: 'model', parts: [{ text: finalResponse }] });
        ...
        // Notify UI
        chrome.runtime.sendMessage({ action: 'manager_progress', text: 'משימה הושלמה' });
        ...
        chrome.runtime.sendMessage({ action: 'manager_progress', text: 'בשגיאה: ' + error.message });
        ...
        try {
            ...
            let hist = hData.convHistory || {};
            if (!hist[conversationId]) hist[conversationId] = [];
            hist[conversationId].push({ role: 'model', parts: [{ text: '❌ ' + error.message }] });
            await chrome.storage.local.set({ convHistory: hist });
        } finally {
            storageMutex.unlock();
        }
