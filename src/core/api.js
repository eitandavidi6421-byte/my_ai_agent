 ...
 */
export async function callGeminiAPI(systemPrompt, messages, options = {}, retries = 3, delay = 2000) {
    ...
    try {
        ...
        if (!response.ok) {
            if ([429, 500, 503].includes(response.status) && retries > 0) {
                ...
                return callGeminiAPI(systemPrompt, messages, options, retries - 1, delay * 2);
            }
        ...
        if (!text) {
            ...
            if (retries > 0 && !options.useSearch) {
                console.warn('Empty Gemini response, retrying without responseMimeType...');
                await new Promise(resolve => setTimeout(resolve, delay));
                return callGeminiAPI(systemPrompt, messages, { ...options, useSearch: true }, retries - 1, delay * 2);
            }
            throw new Error('תגובה ריקה מ-Gemini (finishReason: ' + (finishReason || 'unknown') + ')');
        }
